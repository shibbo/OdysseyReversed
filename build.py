...
def main(argc, argv):
	required_commands = ['make', 'aarch64-none-elf-g++', 'aarch64-none-elf-objdump', 'python']
	...
	if 'make' in argv:
		...
		subprocess.run("wine tools/nsnsotool.exe OdysseyReversed.nso", shell=True)
		...
		with open("OdysseyReversed.nso", 'r+b') as nso_file:
			...
			nso_file.seek(mod0_offset + 0x24)
	...
	if 'ips' in argv:
		...
		with open('output.sym', 'w') as f:
			p = subprocess.Popen('aarch64-none-elf-objdump -t OdysseyReversed.elf', stdout=f, stderr=f, shell=True)
			p.communicate()
			p.wait()
	...
	if 'ftp' in argv:
		ftp.do_connect('subsdk1', 'B424BE150A8E7D78701CBE7A439D9EBF.ips')
